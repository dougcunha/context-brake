...
1. Resolve `tasks/prd-[slug]/prd.md` and `techspec.md`. When `context-snapshot.md` exists there, load it through the Load branch of `.agents/skills/sdd-snapshot/SKILL.md` first. Require the PRD; if missing, point to `sdd-create-prd`. Read the PRD once per version. Reuse an existing TechSpec; update only when authorized, preserving IDs.
