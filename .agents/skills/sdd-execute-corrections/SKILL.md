...
description: SDD execution when tasks from a code review or QA report need correction; implements each correction task in this session with read-only explorers and a pause between tasks; does not create or reclassify findings.
...
If the caller limits execution to one task, return after step 6 instead of asking: `task-completed` with the next eligible IDs, or `blocked` with evidence. When all tasks are complete, run step 7 before returning. A task return does not end the review or the QA cycle.
