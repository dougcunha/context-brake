...
Use `tasks/prd-[slug]/checkpoint.json` as the single operational index for resumption. `workflow.md` stores human decisions, authorizations, and recovery events with stable IDs; manifest + tasks + handoffs remain execution authority, and reports preserve review and QA history. `context-snapshot.md` carries distilled decisions, learnings, and code pointers between sessions under `.agents/skills/sdd-snapshot/SKILL.md`. The checkpoint references these sources without copying their contents.
