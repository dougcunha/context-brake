...
ContextBrake edits files it does not own: harness configuration files and instruction files such as `CLAUDE.md` and `AGENTS.md`. These rules apply to every code path that creates, changes, or removes those files.
...
- Change only the entries ContextBrake registered and the text between `<!-- CONTEXTBRAKE:START -->` and `<!-- CONTEXTBRAKE:END -->`.
...
- `remove` deletes only what ContextBrake created; plan and checkpoint files are deleted only after explicit confirmation.
