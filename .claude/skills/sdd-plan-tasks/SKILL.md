...
3. Use vertical slices: one reviewable result, implementation, and tests in the same task. Separate foundation only when it unlocks multiple deliveries or enables independent migration. Model acyclic dependencies and file or contract collisions; number new tasks after the largest ID in the root and `done/`.
