...
- **Diretório e caminhos:** comandos rodam com o `cwd` da sessão. Para hooks do repositório, a documentação recomenda resolver o caminho pela raiz do git, como `"$(git rev-parse --show-toplevel)/.codex/hooks/..."`, em vez de `.codex/hooks/...`, porque a Codex pode iniciar numa subpasta. `commandWindows` substitui `command` no Windows; o shell que executa o comando não é documentado, e não há forma exec com `args`. Hooks de projeto ficam em `<repo>/.codex/` e só carregam quando essa camada é confiável.
...
- **Pendências:** comportamento em falha e timeout; cobertura dos hooks na CLI; comando de nova sessão; formato de `injectSteps`; diretório em que os hooks rodam e como caminhos relativos em `command` são resolvidos, já que a documentação não traz placeholder nem variável para a raiz do workspace.
