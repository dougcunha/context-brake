...
- **Antes da ferramenta:** `PreToolUse` nega com `hookSpecificOutput.permissionDecision: "deny"` e `permissionDecisionReason`, ou com exit code 2 e o motivo no stderr.
...
- **Antes da ferramenta:** `hookSpecificOutput.permissionDecision: "deny"` com `permissionDecisionReason`, ou exit code 2. A forma antiga `{"decision": "block", "reason": "..."}` também é aceita, e `updatedInput` com `permissionDecision: "allow"` reescreve a entrada.
...
- **Antes da ferramenta:** `preToolUse`, `beforeShellExecution`, `beforeMCPExecution` e `beforeReadFile` negam com `permission: "deny"`, com `user_message` e `agent_message` opcionais; `preToolUse` também aceita `updated_input`.
...
- **Falhas:** por padrão, crash, timeout ou JSON inválido liberam a ação; `failClosed: true` bloqueia nesses casos.
...
- **Antes da ferramenta:** `preToolUse` decide `permissionDecision` entre `allow`, `deny` e `ask`; `permissionDecisionReason` é obrigatório com `deny`.
- **Depois da ferramenta:** `postToolUse` aceita `additionalContext` e `modifiedResult`, que substitui o resultado da ferramenta e exige `resultType: "success"`; `modifiedResult` é respeitado por hooks de comando e HTTP.
...
- **Antes da ferramenta:** `PreToolUse` retorna `decision` entre `allow`, `deny`, `ask`, `force_ask` e `deny_unless_prior_grant`, com `reason` e `permissionOverrides` opcionais.
