...
while (queue.length > 0) { const item = queue.shift(); if (!item) continue; const path = findPackagePath(item.from, item.name); if (visited.has(path)) continue; visited.add(path); const dependency = lock.packages[path]; if (dependency.hasInstallScript) throw new Error(`Runtime dependency has an install script: ${path}`); for (const name of Object.keys(dependency.dependencies ?? {})) queue.push({ from: path, name }); }
