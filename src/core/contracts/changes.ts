...
export type HarnessInstallPlan = { harness: HarnessId; outcome: PlanOutcome; supportLevel: SupportLevel; limitations: readonly CapabilityLimitation[] };
...
const capabilityLimitationSchema = z.object({ capability: z.enum(CAPABILITY_IDS), impact: z.string() }).strict();
export const harnessInstallPlanSchema = z.object({ harness: z.enum(HARNESS_IDS), outcome: z.enum(PLAN_OUTCOMES), supportLevel: z.enum(SUPPORT_LEVELS), limitations: z.array(capabilityLimitationSchema) }).strict();
