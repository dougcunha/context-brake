...
export async function resolveGuidance(sources: GuidanceSources): Promise<ZoneGuidance> {
  const mode = await resolveCheckpointMode(sources.config, sources.planPresence);
  return guidanceForMode(mode, sources);
}
...
function guidanceForMode(mode: CheckpointMode, sources: GuidanceSources): ZoneGuidance {
  const section = sources.config.delegatedSnapshot;
  return mode === 'delegated' && section !== undefined ? delegatedGuidance(sources.config, section) : planGuidance(sources);
}
export function planGuidance(sources: GuidanceSources): ZoneGuidance {
  const { config } = sources;
  return {
    mode: 'plan',
    actionFor: (zone) => ZONE_ACTIONS[zone].compact,
