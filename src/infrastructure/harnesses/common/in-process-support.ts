...
async function resolveInProcessFailure(projectRoot: string, event: RuntimeEvent, error: unknown): Promise<RuntimeDecision> {
  ...
  return await resolveFailure({ event, code: failureErrorCode(error), detail: failureDetail(error), config: null, descriptor: null, ledger: ports.ledger, errors: ports.errors, readValidationCommand: ports.readValidationCommand, planPresence: ports.planPresence });
}
