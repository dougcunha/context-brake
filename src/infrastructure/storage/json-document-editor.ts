...
export function setJsonProperty(text: string, path: readonly (string | number)[], value: unknown): string {
  const root = parseAndValidateJson(text);
  const indent = detectIndent(text);
  const eol = detectEol(text);
  const format = text.includes('\n') ? indent : '';
  const existingNode = findNodeAtLocation(root, path as (string | number)[]);
  if (existingNode) {
  ...
  const parentNode = parentPath.length === 0 ? root : findNodeAtLocation(root, parentPath as (string | number)[]);
  if (!parentNode) {
...
export function appendJsonArrayItem(text: string, path: readonly (string | number)[], value: unknown): string {
  const root = parseAndValidateJson(text);
  const arrayNode = findNodeAtLocation(root, path as (string | number)[]);
  if (!arrayNode) {
...
export function removeJsonProperty(text: string, path: readonly (string | number)[]): string {
  const root = parseAndValidateJson(text);
...
export function removeJsonArrayItem(text: string, path: readonly (string | number)[], predicate: (item: unknown) => boolean): string {
  const root = parseAndValidateJson(text);
  const arrayNode = findNodeAtLocation(root, path as (string | number)[]);
