...
- CA-20 (Restrição de plataforma): Dados Linux, macOS e Windows, em PowerShell e Git Bash, quando os cenários CA-01, CA-05 e CA-07 são executados, então todos passam.
  - Exceção registrada (decisão do responsável pelo produto, 2026-09-14): não há máquina macOS nem distribuição Linux além do WSL 2 disponíveis. A aceitação deste critério usa evidência de Linux (Ubuntu no WSL 2) e de Windows, em PowerShell e Git Bash, com Node 20, 22 e 24, a partir de uma única revisão. A execução em macOS não é exigida para aceitar este PRD e fica registrada como não verificada. Detalhes em `DEC-01` na [TechSpec](./techspec.md).
...
- **Plataformas:** Linux, macOS e Windows, em PowerShell e Git Bash. O macOS continua como plataforma-alvo, mas não tem evidência de aceitação neste PRD; ver a exceção registrada no CA-20.
