...
| FR-02 | Derivar o nível de suporte por esta regra:<br>- **Completo:** o harness respeita a negação explícita do hook em todas as chamadas de ferramenta e entrega contexto após a ferramenta e no início da sessão.<br>- **Parcial:** o harness bloqueia por negação explícita, mas alguma dessas capacidades falta, é indireta, não está confirmada ou não cobre todas as ferramentas.<br>- **Cooperativo:** o harness não bloqueia.<br><br>Timeout e falha da integração não rebaixam o nível. | `init` e `doctor` mostram Claude Code, Cursor, GitHub Copilot CLI, Pi e Oh-My-Pi como completos. Codex CLI e OpenCode aparecem como parciais — ferramentas hospedadas ignoram os hooks e o contexto após a ferramenta não está confirmado, respectivamente. Antigravity CLI aparece como cooperativo, porque o PRD-01 instala apenas o hook `PreInvocation` e não registra `PreToolUse`, então a negação seletiva por ferramenta só chega com o PRD-02. |
