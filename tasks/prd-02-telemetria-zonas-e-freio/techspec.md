...
- Dependent PRDs: `tasks/prd-01-instalacao-deteccao-diagnostico/prd.md` (capability matrix, support levels, `doctor`) and its `techspec.md`; `tasks/prd-03-plano-checkpoint-e-boot/prd.md` (plan file with validation commands); `tasks/prd-04-runner-de-reinicio-automatico/prd.md` (out of scope, `wrap`).
...
| DEC-10 | RF21, CA-17, US6 | The brake mode is derived from capability definitions: `enforced` when `pre_tool_block` and `timeout_fail_closed` are both `supported`, otherwise `cooperative`, with the first missing capability's `impact` as the reason. Each harness's `CAPABILITIES` moves to `capabilities.ts`, imported by both `adapter.ts` and `runtime.ts`. The runtime writes the mode and reason in the ledger's `session` line. Doctor emits one `BRAKE_COOPERATIVE` warning per harness with recorded cooperative sessions, naming up to five of the most recent session IDs and the reason. `DoctorReport` schema v1 is unchanged. | This keeps a single source for PRD-01 support levels and PRD-02 brake guarantees. Findings already carry harness, message, and impact, and adding a report field would break strict consumers of the published schema. | Recompute the mode in doctor: loses what the session actually ran with after an upgrade. Add a `sessions` array to `DoctorReport`: a published schema change. |
| DEC-11 | RF8, honest-measurement objective | `support-service.ts` derives `full` from `pre_tool_block`, `post_tool_telemetry`, `session_boot`, and `timeout_fail_closed`, excluding `context_usage`. Claude Code and Cursor declare `context_usage` as `unsupported`, with impacts saying usage is estimated. | PRD-01 defines full support as a guaranteed block, telemetry, and boot, with no mention of usage. Research and the README say Claude Code usage reaches only the status line and Cursor usage only `preCompact`, yet both adapters currently claim `context_usage: supported` because `allCapabilities` requires all five. No harness changes support level. | Keep the claims: doctor would report measured usage while blocks say `source=estimated`. Mark the capability unsupported without changing the derivation: Claude Code and Cursor would drop to `partial`, contradicting PRD-01. |
...
| CMP-15 | `src/core/services/support-service.ts` | Modified | Full support uses the four guarantee capabilities (DEC-11). | — |
| CMP-16 | `src/core/services/doctor-service.ts`, `src/core/contracts/adapter.ts` | Modified | Accept brake session summaries and add their findings; add `BenchmarkFixture.event`. | CMP-11 |
...
| CMP-23 | `src/infrastructure/harnesses/*/adapter.ts` (8) | Modified | Import `CAPABILITIES` from `capabilities.ts`; update benchmark fixtures to documented payloads plus `event`. | CMP-20, CMP-16 |
| CMP-24 | `src/infrastructure/diagnostics/overhead-measurer.ts`, `src/cli/commands/doctor.ts` | Modified | Pass the event argument; pick the tool handler with a mock context; read brake sessions and logs for doctor. | CMP-16, CMP-17 |
| CMP-25 | `assets/runtime/*.ts`, `scripts/asset-bundler.ts`, `tsconfig.check.json`, `package.json`, `tests/test-lanes.ts` | Modified | Thin entrypoints; expose the esbuild metafile for the bundle guard; typecheck assets; add devDependency `js-tiktoken`; register new process-lane tests. `assets/runtime/process-hook.ts` is deleted. | CMP-19, CMP-20 |
...
`BenchmarkFixture` gains a required `event: string`. All eight adapters and the doctor test fakes are updated.
...
| 2. Pure core: CMP-01 to CMP-11, CMP-15 | 1 | TC-01, TC-02, TC-05 to TC-07, TC-10, TC-11, TC-15 to TC-17, TC-21, TC-25, TC-28, TC-30 pass with port fakes. |
...
| 7. Doctor wiring, measurer, assets, bundler, typecheck scope: CMP-24, CMP-25 | 5, 6 | TC-19, TC-22, and TC-24 pass; `npm run build`, `assets:check`, and `package:smoke` pass. |
...
| TC-19 | RF21, CA-17, DEC-10 | unit and integration | Copilot ledgers for three sessions and a Claude Code ledger; run doctor | One `BRAKE_COOPERATIVE` warning for Copilot, with its session IDs and the timeout reason; none for Claude Code; identical text and JSON findings | `tests/unit/brake-session-checks.test.ts`, `tests/integration/doctor-brake-sessions.test.ts` |
...
| TC-25 | RF8, DEC-11 | unit | Profiles for all eight adapters and all capability combinations | Claude Code, Cursor, Pi, Oh-My-Pi are `full`; the others `partial`; `unknown` in any of the four guarantee capabilities is never `full` | `tests/unit/support-service.test.ts` |
...
Measured on 2026-09-14 at `99643a5` with the commands from `references/preparatory-refactoring.md` and the QA commands above. No target file has a declaration with 4+ parameters or any `case` statement, and none triggered QA-01 to QA-06 or QA-08.
...
- Open item OI-07: PRD-01 follow-ups: whether `remove` deletes `.context-brake/runtime/`, and whether doctor flags installed assets that differ from the packaged version, so users upgrading from the stubs learn to rerun `init`. Owner: PRD-01 owner. Affects rollout.
