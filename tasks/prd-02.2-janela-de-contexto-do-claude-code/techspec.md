...
| DEC-13 | NFR-01 | The bridge budget is measured like the hook budget, in `tests/integration/statusline-overhead.test.ts`. The bridge runs in `--pipe` mode feeding `node -e "process.stdin.resume()"`, against the same user command alone. The test also measures the p95 of an empty `node -e` (one Node start). In CI, the p95 difference must be at most 50 ms plus that Node start; hooks with 200 `statusline` lines must stay within 120 ms over the empty-Node baseline. Locally, each must be at most `max(target, baseline*3 + 150)`. Each case uses 20 samples after 3 warmups, as existing cases do. Amended 26/09/2026 (DEC-HIL-05 in `workflow.md`, finding codereview_01/CR-04): CI run 36256275030 measured the bridge at +62 to +68 ms on Windows and macOS, where an empty Node start alone takes 42 to 85 ms. | This reuses the p95 sampler and the CI/local split of existing cases (`runtime-overhead.test.ts:13-59`). Measuring against the user command isolates what the bridge adds, and the Node start term keeps the rule honest on slower runners. | A flat 50 ms fails on every Windows runner for any Node-based bridge. Moving the bridge off the critical path (background recording) was rejected for now: it risks losing recordings and makes the command harder to keep correct. |
...
| TC-20 | NFR-01, OBJ-04 | integration | Bridge overhead (DEC-13) and hook overhead with 200 `statusline` lines in the ledger | Bridge p95 within 50 ms plus one Node start; hooks within 120 ms | `tests/integration/statusline-overhead.test.ts` |
