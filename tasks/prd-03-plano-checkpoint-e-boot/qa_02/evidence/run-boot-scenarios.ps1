...
$Evidence = Join-Path $Repo 'tasks\prd-03-plano-checkpoint-e-boot\qa_02\evidence'
...
$MeasureTokens = Join-Path $Evidence 'measure-tokens.mjs'
$AssetDir = Join-Path $Repo 'dist\assets\runtime'
$TmpRoot = Join-Path $env:TEMP 'cb-qa-02-boot'
...
$script:log = $log
$script:lastRaw = ''
$log.Add("QA boot delivery scenarios (qa_02) - $(Get-Date -Format o)")
...
$ca07Path = Join-Path $Evidence 'ca07-boot.json'
Set-Content -Path $ca07Path -Value $script:lastRaw -Encoding utf8 -NoNewline
$ca07Tokens = node $MeasureTokens $ca07Path | ConvertFrom-Json
$log.Add("TOKENS ca07: tokens=$($ca07Tokens.tokens) bytes=$($ca07Tokens.bytes)")
Assert 'CA-07' 'over-budget boot token count stays within the configured budget' ($ca07Tokens.tokens -le 1000) "tokens=$($ca07Tokens.tokens) bytes=$($ca07Tokens.bytes)"
...
$ca08Path = Join-Path $Evidence 'ca08-boot.json'
Set-Content -Path $ca08Path -Value $script:lastRaw -Encoding utf8 -NoNewline
$ca08Tokens = node $MeasureTokens $ca08Path | ConvertFrom-Json
$log.Add("TOKENS ca08: tokens=$($ca08Tokens.tokens) bytes=$($ca08Tokens.bytes)")
Assert 'CA-08' '20/20/20 boot is within 1,000 tokens (js-tiktoken o200k_base)' ($ca08Tokens.tokens -le 1000) "tokens=$($ca08Tokens.tokens) bytes=$($ca08Tokens.bytes)"
