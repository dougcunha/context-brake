...
$TmpRoot = Join-Path $env:TEMP 'cb-qa-02-cli'
if (Test-Path $TmpRoot) { Remove-Item -Recurse -Force $TmpRoot }
New-Item -ItemType Directory -Path $TmpRoot | Out-Null
$log = New-Object System.Collections.Generic.List[string]
$script:failures = 0
$script:log = $log
$log.Add("QA CLI scenarios (qa_02) - $(Get-Date -Format o)")
...
Assert 'CA-03' 'existing invalid plan is never reported as missing and never suggests plan init' ((-not (Contains $badText 'No plan exists')) -and (-not (Contains $badText 'plan init'))) "out=$($r4.stdout) err=$($r4.stderr)"

$fxMissing = Join-Path $TmpRoot 'fx-noplan'
New-Item -ItemType Directory -Path $fxMissing | Out-Null
node $WriteConfig $fxMissing $Repo | Out-Null
$rMissing = Invoke-Cli $fxMissing @('plan', 'status')
$missingText = "$($rMissing.stdout) $($rMissing.stderr)"
Assert 'T17' 'absent plan keeps the healthy missing-plan guidance' (($rMissing.code -eq 0) -and (Contains $missingText 'No plan exists at task_plan.json') -and (Contains $missingText 'plan init')) "exit=$($rMissing.code) out=$($rMissing.stdout) err=$($rMissing.stderr)"
