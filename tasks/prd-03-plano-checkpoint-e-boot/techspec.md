...
| DEC-02 | RF9 | Widen the `context` event gate in `claude-code/runtime.ts:55`, `codex-cli/runtime.ts:64`, and `cursor/runtime.ts:45` to also emit on the session-start event name. | Those three gate on `PostToolUse`/`postToolUse`; `github-copilot-cli/runtime.ts:61` has no event gate and needs no change. | Removing the gate entirely would emit telemetry on unintended events. |
...
| DEC-04 | RF9 | For Pi and Oh-My-Pi, return the boot from the existing session-start handlers, which today discard the decision (`pi/runtime.ts:58-61`, `oh-my-pi/runtime.ts:58-61`), delivering it through the documented `before_agent_start` `message` channel. | Both declare `session_boot: supported` (`capabilities.ts:7`); `harness-integrations.md` §Pi documents `before_agent_start` returning `message`/`systemPrompt` and states the ContextBrake boot arrives in PRD-03. | `pi.sendMessage` delivery modes are documented but add an ordering guarantee the docs do not pin down. |
...
| CMP-17 | `src/infrastructure/harnesses/{claude-code,codex-cli,cursor}/runtime.ts` | modified | Emit `additionalContext` / `additional_context` on session start | DEC-02 |
| CMP-18 | `src/infrastructure/harnesses/{pi,oh-my-pi}/runtime.ts` | modified | Return the boot from the session-start handler | DEC-04 |
...
- **Session start, process hooks.** Claude Code, Codex CLI, Cursor, and GitHub Copilot CLI already register the event and already parse the payload; only the `renderDecision` gate changes for the first three. Output stays the documented field for that event, per `harness-adapters.md`.
- **Session start, in-process.** Pi and Oh-My-Pi return the boot from the existing handler. No synchronous I/O is introduced on these paths, per `node.md`.
...
- **Failure policy.** A boot failure never throws into the harness: the hook host's existing deadline and failure path apply, and a boot that cannot be built degrades to `NEUTRAL`, leaving the session usable.
...
| TC-05 | RF9, CA-05 | integration | Step 3 in progress, two constraints | Boot carries task, steps 3 and 4, both constraints, validation command | `tests/integration/boot-summary.test.ts` |
...
- Modify: `src/core/services/brake-engine.ts`, `zone-actions.ts`, `protocol-service.ts`, `doctor-checks.ts`; `src/cli/argument-parser.ts`, `composition-root.ts`, `main.ts`, `output/text.ts`; `src/infrastructure/harnesses/{claude-code,codex-cli,cursor,pi,oh-my-pi}/runtime.ts`; `scripts/generate-schemas.ts`, `check-schemas.ts`, `check-package.ts`; `tests/test-lanes.ts`.
