...
function isInProcess(harness: ProcessHarnessId | InProcessHarnessId): harness is InProcessHarnessId { return harness === 'pi' || harness === 'oh-my-pi'; }
...
async function createRoot(window: SimulatedWindow): Promise<string> {
  ...
  await writeFile(join(root, '.gitignore'), `${PLAN_FILE}\n${CHECKPOINT_FILE}\n`, 'utf8');
...
async function assertSession(input: { readonly root: string; readonly harness: HarnessId; readonly sessionId: string; readonly recorder: SessionRecorder; readonly profile: AgentProfile }): Promise<void> {
  ...
  const checkpoint = parseStateCheckpoint(JSON.parse(await readFile(join(input.root, CHECKPOINT_FILE), 'utf8')));
  const plan = parseTaskPlan(JSON.parse(await readFile(join(input.root, PLAN_FILE), 'utf8')));
  assertCheckpointMatchesPlan(checkpoint, plan);
  expect(await lastCommitSubject(input.root), summary).toBe('checkpoint: step 1');
  const committed = await runGit(['diff-tree', '--no-commit-id', '--name-only', '-r', 'HEAD'], input.root);
  expect(committed, summary).not.toContain(PLAN_FILE);
  expect(committed, summary).not.toContain(CHECKPOINT_FILE);
  expect((await runGit(['status', '--porcelain'], input.root)).trim(), summary).toBe('');
