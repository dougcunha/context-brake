...
export async function writeInvalidRuntimeConfig(projectRoot: string): Promise<void> {
  const config = { ...DEFAULT_CONFIG, telemetry: { ...DEFAULT_CONFIG.telemetry, turnCeiling: 11 } };
  await writeFile(join(projectRoot, 'context-brake.config.json'), JSON.stringify(config), 'utf8');
}
...
function toolLine(turn: number): ToolLineInput {
  return { toolUseId: `seed-${turn}`, observedCharacters: 0, turn, usedTokens: 16000, windowTokens: 24000, estimatedTokens: 16000, source: 'estimated', zone: 'CRITICAL' };
}
