...
function renderBoundaries(config: ContextBrakeConfig): Boundaries {
  ...
  return { greenMaxPercentage: (green[0] ?? 0) - 1, yellowMinPercentage: yellow[0] ?? 0, yellowMaxPercentage, redMinPercentage: yellowMaxPercentage + 1, criticalMinPercentage: critical[0] ?? 0, redTurn: rowNumbers(config, 'RED')[1] ?? null };
}
function withZones(config: ContextBrakeConfig, zones: ContextBrakeConfig['telemetry']['zones']): ContextBrakeConfig {
  return { ...config, telemetry: { ...config.telemetry, zones } };
}
function customZones(): ContextBrakeConfig['telemetry']['zones'][] {
  return [
    { greenMaxPercentage: 19, yellowMaxPercentage: 39, criticalPercentage: 59, greenMaxTurn: 5, yellowMaxTurn: 9 },
    { greenMaxPercentage: 60, yellowMaxPercentage: 70, criticalPercentage: 95 },
  ];
}
function expectCoherent(config: ContextBrakeConfig): void {
  ...
  expectTurnCoherence(zones, rendered.redTurn);
  expect(zoneRow(config, 'CRITICAL')).not.toContain('turns');
  expect(classifyZone({ usagePercentage: 0, turns: 100000 }, zones)).not.toBe('CRITICAL');
}
function expectTurnCoherence(zones: ContextBrakeConfig['telemetry']['zones'], redTurn: number | null): void {
  if (zones.yellowMaxTurn === undefined) {
    expect(redTurn).toBeNull();
    return;
  }
  expect(redTurn).toBe(zones.yellowMaxTurn + 1);
  expect(classifyZone({ usagePercentage: 0, turns: zones.yellowMaxTurn + 1 }, zones)).toBe('RED');
  expect(classifyZone({ usagePercentage: 0, turns: zones.yellowMaxTurn }, zones)).toBe('YELLOW');
}
